...
import { links } from "../data/widget-init";
...
function Sidebar() {
  ...
  const normalLink =
    "flex items-center gap-5 pl-4 pt-3 pb-2.5 rounded-lg  text-md text-gray-700 dark:text-gary-200 dark:hover:text-black hover:bg-light-gary m-2";
  return (
      ...
      {activeMenu && (
        <>
          ...
          <div className="mt-10">
            {links.map((item) => (
              <div key={item.title}>
                <p className="text-gray-400 dark:text-white-400 m-3 mt-4 uppercase">
                  {item.title}
                </p>
                {item.links.map((link) => (
                  <NavLink
